...
#%%
class CanBerth(dtv_backend.logbook.HasLog):
    """
    ...
    def __compute_eta_per_berth(self, src_node, dst_node, berth_nodes, mean_speed):
        ...
        for berth in berth_nodes:
            ...
            row = {'berth': berth,
                   ...
                   'eta': self.__time_now + datetime.timedelta(seconds=duration_src_to_node),
    ...
    def find_berth(self, src_node, dst_node, max_timestamp,
                   max_distance, mean_speed, include_dst=True):
        """
        ...
        berths = self.__find_berths_near_route(src_node=src_node,
                                               dst_node=dst_node,
                                               max_distance=max_distance)
        
        # check if any feasible berths are found, else continue
        if len(berths) == 0:
            print(f'No berths were found on the route from src "{src_node}" '\
            ...
            eta_df = self.__compute_eta_per_berth(src_node=src_node,
                                                dst_node=dst_node,
                                                berth_nodes=berths,
                                                mean_speed=mean_speed)
            
            # use the output to suggest the best berth
            berth = self.__find_best_berth(df=eta_df,
